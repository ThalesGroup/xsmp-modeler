...
import { toXsmpIdentifier } from '../../../../language/utils/path-utils.js';
...
export function parseSmpXml(xml: string): ParsedSmpXmlDocument {
    ...
    const [rootKey] = Object.keys(parsed);
    if (!rootKey) {
        throw new Error('Parsed SMP XML does not contain a root element.');
    }

    const root = parsed[rootKey];
    if (!isSmpXmlObject(root)) {
        throw new Error(`Parsed SMP XML root '${rootKey}' is malformed.`);
    }
