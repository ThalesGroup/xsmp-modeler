...
export function collectDiagnostics(documents: readonly LangiumDocument[]): CliDiagnostic[] {
    const diagnostics = new Map<string, CliDiagnostic>();

        for (const document of documents) {
        for (const parserError of document.parseResult.parserErrors) {
            const diagnostic = toCliParserErrorDiagnostic(document, parserError);
            diagnostics.set(createDiagnosticKey(diagnostic), diagnostic);
        }
        for (const diagnostic of document.diagnostics ?? []) {
            const cliDiagnostic = toCliDiagnostic(document, diagnostic, toSeverityLabel(diagnostic.severity));
...
function toCliDiagnostic(document: LangiumDocument, diagnostic: Diagnostic, severity: CliDiagnostic['severity']): CliDiagnostic {
    return {
        document,
...
function toCliParserErrorDiagnostic(document: LangiumDocument, diagnostic: unknown): CliDiagnostic {
