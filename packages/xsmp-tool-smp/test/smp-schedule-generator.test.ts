...
let parse: ReturnType<typeof parseHelper<ast.Schedule>>;
...
beforeAll(async () => {
  ...
  parse = parseHelper<ast.Schedule>(services.xsmpsed);
...
async function parseInProject(source: string): Promise<LangiumDocument<ast.Schedule>> {
...
`,
  ...
  await rebuildTestDocuments(services, [projectDocument, catalogueDocument, scheduleDocument], false);
...
function checkDocumentValid(document: LangiumDocument): string | undefined {
  return document.parseResult.parserErrors.length && s`
    ...
    || !ast.isSchedule(document.parseResult.value) && `Root AST object is a ${document.parseResult.value.$type}, expected a '${ast.Schedule.$type}'.`
    || undefined;
}
