...
async function parseFixture(fileName: string): Promise<LangiumDocument<ast.Assembly>> {
    return parseSource(fs.readFileSync(path.resolve(__dirname, fileName)).toString(), fileName);
}

async function parseSource(source: string, fileName = 'demo.xsmpasb'): Promise<LangiumDocument<ast.Assembly>> {
...
`,
    ...
    await rebuildTestDocuments(services, [projectDocument, catalogueDocument, assemblyDocument], false);
...
function checkDocumentValid(document: LangiumDocument): string | undefined {
    ...
    `
        ...
        || !ast.isAssembly(document.parseResult.value) && `Root AST object is a ${document.parseResult.value.$type}, expected a '${ast.Assembly.$type}'.`
        || undefined;
}
