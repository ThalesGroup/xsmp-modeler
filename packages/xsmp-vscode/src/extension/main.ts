...
const xsmpLanguageIdSet = new Set<string>(xsmpLanguageIds);
...
function shouldGenerateOnSave(document: vscode.TextDocument): boolean {
    ...
    if (!xsmpLanguageIdSet.has(document.languageId)) {
        return false;
    }
...
async function startLanguageClient(context: vscode.ExtensionContext): Promise<LanguageClient> {
    ...
    const serverModule = context.asAbsolutePath(path.join('out', 'language', 'main.cjs')),
        ...
        clientOptions: LanguageClientOptions = {
            documentSelector: createXsmpDocumentSelector(),
            markdown: {
...
function createXsmpFileWatchers(context: vscode.ExtensionContext): vscode.FileSystemWatcher[] {
    return xsmpFileWatcherPatterns.map(pattern => {
