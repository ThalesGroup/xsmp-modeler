...
`);
        ...
        await services.shared.ContributionRegistry.registerExtensionManifestEntries([contributionEntry]);

        await services.shared.workspace.WorkspaceManager.initializeWorkspace([
            { name: 'dep', uri: URI.file(depProjectDir).toString() },
            { name: 'app', uri: URI.file(appProjectDir).toString() },
            { name: 'isolated', uri: URI.file(isolatedProjectDir).toString() },
        ]);
...
`);
        ...
        const report = await services.shared.ContributionRegistry.registerExtensionManifestEntries(entries);
