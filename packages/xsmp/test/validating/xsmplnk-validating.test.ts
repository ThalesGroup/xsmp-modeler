...
import { Assembly, Catalogue, LinkBase, Project, isLinkBase } from '@xsmp/core/ast-partial';
...
`,
    ...
    expect(assemblyDocument?.parseResult.parserErrors ?? []).toHaveLength(0);
    expect(linkBaseDocument.parseResult.parserErrors).toHaveLength(0);
    await rebuildTestDocuments(services, [projectDocument, catalogueDocument, ...(assemblyDocument ? [assemblyDocument] : []), linkBaseDocument]);
