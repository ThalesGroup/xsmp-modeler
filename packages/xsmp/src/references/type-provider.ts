...
export class XsmpTypeProvider {
    ...
    private doGetType(node: AstNode): ast.Type | undefined {
        if (ast.isField(node.$container) || ast.isConstant(node.$container) || ast.isProperty(node.$container) || ast.isParameter(node.$container) || ast.isReturnParameter(node.$container)) {
