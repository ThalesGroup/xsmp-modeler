...
export class SmpImportService {
    ...
    async importFile(request: SmpImportRequest): Promise<SmpImportResult> {
        ...
        await this.awaitWorkspaceReady();
    ...
    async renderImportedDocument(request: {
        readonly inputPath: string;
        readonly outputUri: URI;
        readonly workspaceIndex?: SmpWorkspaceIndex;
    }): Promise<SmpRenderImportResult> {
        await this.services.ContributionRegistry.ready;
    ...
    protected async awaitWorkspaceReady(workspaceIndex?: SmpWorkspaceIndex): Promise<void> {
        if (workspaceIndex) {
            return;
        }
