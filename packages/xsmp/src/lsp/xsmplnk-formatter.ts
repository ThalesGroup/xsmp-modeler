...
export class XsmplnkFormatter extends XsmpFormatterBase {
    protected override format(node: AstNode): void {
        switch (node.$type) {
            ...
            case ast.LinkBase.$type: return this.formatLinkBase(node as ast.LinkBase, this.getNodeFormatter(node));
            case ast.ComponentLinkBase.$type: return this.formatComponentLinkBase(node as ast.ComponentLinkBase, this.getNodeFormatter(node));
    ...
    protected formatLinkBase(node: ast.LinkBase, formatter: NodeFormatter<ast.LinkBase>): void {
        ...
        formatter.property(ast.LinkBase.name).append(Formatting.newLine({ allowMore: true }));
